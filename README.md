...
- This SDK is designed to facilitate the use of FQS.
...
- Store the name of the created queue and the issued SecretKey locally. **Never expose these externally!**
...
```
- You can use the SDK with the above `build.gradle` configuration.
- Replace the ${version} variable with the desired version number!
- Since it is distributed via Jitpack, the `maven { url 'https://jitpack.io' }` step is essential.

### Java-SDK
- An example code is implemented using the Spring framework in a Java-based environment.
  - Other frameworks are not yet included due to inexperience.
```java
@Configuration
...
```
- In Spring Boot, you can inject the queue name and SecretKey as variables from the `application.yml` or `application.properties` file.
  - Ensure that these values are not exposed externally in any other frameworks.
- The FQSSdk is designed to be registered as a Bean, making it easy to use through dependency injection in your business logic.

### Spring-SDK
- A Spring Boot-based SDK has also been implemented.
- It has been developed with minimal dependencies to reduce external influences.
...
- You can configure the application files according to your preferences.
- However, make sure to maintain the hierarchical structure for proper functioning.
...
```
- You can easily use the FQSSdk interface as shown above.
- Feel free to add your own business logic before and after using FQSSdk for diverse functionalities.

## ❤️ Contributing
- Interested in contributing to FQS-sdk? Follow these steps :)
...
- The maintainers will review the PR and decide whether to merge it after evaluation.

## 📪 Contact
- If you have any questions or feedback regarding FQS-sdk, please reach out via email: kw4u1223@gmail.com.
